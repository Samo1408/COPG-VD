...
});var tp=document.getElementById('tab-device-profile');var ts=document.getElementById('tab-sim-gps-spoof');var ti=document.getElementById('tab-ids-spoof');if(tp)tp.addEventListener('click',function(){tp.classList.add('active');if(ts)ts.classList.remove('active');if(ti)ti.classList.remove('active');document.getElementById('device-profile-section').classList.add('active');document.getElementById('sim-gps-section').classList.remove('active');document.getElementById('ids-section').classList.remove('active')});if(ts)ts.addEventListener('click',function(){ts.classList.add('active');if(tp)tp.classList.remove('active');if(ti)ti.classList.remove('active');document.getElementById('sim-gps-section').classList.add('active');document.getElementById('device-profile-section').classList.remove('active');document.getElementById('ids-section').classList.remove('active')});if(ti)ti.addEventListener('click',function(){ti.classList.add('active');if(tp)tp.classList.remove('active');if(ts)ts.classList.remove('active');document.getElementById('ids-section').classList.add('active');document.getElementById('device-profile-section').classList.remove('active');document.getElementById('sim-gps-section').classList.remove('active')});var cs=document.getElementById('sim-carrier-select');if(cs)cs.addEventListener('change',onCarrierSelectChange);var ab=document.getElementById('apply-sim-gps');if(ab)ab.addEventListener('click',applySimGpsSpoof);
...
function randomHexId(){var c='0123456789abcdef',s='';for(var i=0;i<16;i++)s+=c[Math.floor(Math.random()*c.length)];return s;}
function randomUuid(){if(window.crypto&&crypto.randomUUID)return crypto.randomUUID();var b=[];for(var i=0;i<16;i++)b.push(Math.floor(Math.random()*256));b[6]=(b[6]&15)|64;b[8]=(b[8]&63)|128;var h=b.map(function(x){return x.toString(16).padStart(2,'0')}).join('');return h.slice(0,8)+'-'+h.slice(8,12)+'-'+h.slice(12,16)+'-'+h.slice(16,20)+'-'+h.slice(20);}
async function loadInstalledAppsForIds(){
    var s=document.getElementById('ids-app-select'); if(!s)return;
    try{
        var out=await execCommand("pm list packages -3 2>/dev/null | sed 's/^package://'");
        var pkgs=out.split(/\s+/).map(function(x){return x.trim()}).filter(Boolean).sort();
        s.innerHTML='<option value="">-- Select an installed app --</option>';
        pkgs.forEach(function(p){var o=document.createElement('option');o.value=p;o.textContent=p;s.appendChild(o)});
    }catch(e){appendToOutput('Could not load installed apps: '+e,'warning');}
}
function loadSelectedIdProfile(){
    var p=document.getElementById('ids-app-select').value, d=(currentConfig['COPG-VD-IDs']||{})[p]||{};
    document.getElementById('android-id-value').value=d.android_id||'';
    document.getElementById('gsf-id-value').value=d.gsf_id||'';
    document.getElementById('app-set-id-value').value=d.app_set_id||'';
    document.getElementById('toggle-ids-spoof').checked=d.enabled===true;
}
async function saveIdProfile(){
    var p=document.getElementById('ids-app-select').value;if(!p){appendToOutput('Select an application first','error');return}
    var a=document.getElementById('android-id-value').value.trim().toLowerCase();
    var g=document.getElementById('gsf-id-value').value.trim().toLowerCase();
    var u=document.getElementById('app-set-id-value').value.trim();
    if(a && !/^[0-9a-f]{16}$/.test(a)){appendToOutput('Android ID must be exactly 16 hexadecimal characters','error');return}
    if(g && !/^[0-9a-f]{16}$/.test(g)){appendToOutput('GSF ID must be exactly 16 hexadecimal characters','error');return}
    if(u && !/^[0-9a-fA-F-]{16,64}$/.test(u)){appendToOutput('App Set ID format is invalid','error');return}
    if(!currentConfig['COPG-VD-IDs'])currentConfig['COPG-VD-IDs']={};
    currentConfig['COPG-VD-IDs'][p]={enabled:document.getElementById('toggle-ids-spoof').checked,android_id:a,gsf_id:g,app_set_id:u};
    if(!configKeyOrder.includes('COPG-VD-IDs'))configKeyOrder.push('COPG-VD-IDs');
    await saveConfig();appendToOutput('ID profile saved for '+p,'success');
}
function setupIdsUi(){
    var s=document.getElementById('ids-app-select');if(!s)return;
    s.addEventListener('change',loadSelectedIdProfile);
    document.getElementById('random-android-id-btn').addEventListener('click',function(){document.getElementById('android-id-value').value=randomHexId()});
    document.getElementById('random-gsf-id-btn').addEventListener('click',function(){document.getElementById('gsf-id-value').value=randomHexId()});
    document.getElementById('random-app-set-id-btn').addEventListener('click',function(){document.getElementById('app-set-id-value').value=randomUuid()});
    document.getElementById('apply-ids').addEventListener('click',saveIdProfile);
    loadInstalledAppsForIds();
}
