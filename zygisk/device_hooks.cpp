...
static void installWifi(JNIEnv* e,const DeviceCfg& c) {
    ...
    if (ssid != "<unknown ssid>" && ssid != "unknown ssid" && !(ssid.size() >= 2 && ssid.front() == '"' && ssid.back() == '"')) ssid = "\"" + ssid + "\"";
    jobject v=newString(e,ssid); if(m&&v){jobject h=makeConstantHooker(e,v,1); if(h){jobject b=lsplant::Hook(e,m,h,cb); if(b){g_hookers.push_back(e->NewGlobalRef(h));LOGI("hooked WifiInfo.getSSID");}}}
}
