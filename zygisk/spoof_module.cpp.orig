...
public:
    ...
    static std::string s_media_drm_level;
    static std::string s_sim_iso;
    static std::string s_sim_carrier;
    static std::string s_sim_mccmnc;
    static bool s_sim_spoof_enabled;

    // MediaDrm.getPropertyByteArray() has one standard byte-array property:
    // "deviceUniqueId". Unlike WifiInfo#getSSID(), this API is genuinely JNI-native
    // on AOSP, so hookJniNativeMethods can replace it without pretending a Java method
    // is native. We only override that one property; other byte-array properties are
    // outside the contract and return null rather than silently fabricating DRM data.
    static jbyteArray hooked_getPropertyByteArray(JNIEnv* env, jobject /* thiz */, jstring name) {
        if (!name || s_media_drm_id.empty()) return nullptr;
        const char* raw = env->GetStringUTFChars(name, nullptr);
        if (!raw) return nullptr;
        const std::string property(raw);
        env->ReleaseStringUTFChars(name, raw);
        if (property != "deviceUniqueId") return nullptr;

        std::string value = trim(s_media_drm_id);
        std::string hex;
        ...
        bool valid_hex = !hex.empty() && (hex.size() % 2 == 0);
        for (char c : hex) {
        ...
        if (valid_hex) {
            ...
            for (size_t i = 0; i < hex.size(); i += 2) {
                bytes.push_back(static_cast<jbyte>((nibble(hex[i]) << 4) | nibble(hex[i + 1])));
            }
        } else {
            // The UI accepts a readable token too. In that case expose its UTF-8 bytes
            // instead of corrupting the Java byte-array contract.
            bytes.reserve(value.size());
            for (unsigned char c : value) bytes.push_back(static_cast<jbyte>(c));
        }
        ...
        if (!bytes.empty()) env->SetByteArrayRegion(result, 0, static_cast<jsize>(bytes.size()), bytes.data());
    ...
    void preAppSpecialize(zygisk::AppSpecializeArgs *args) override {
        (void)args;
        if (s_sim_spoof_enabled && !s_media_drm_id.empty()) {
            // This is a real native method on AOSP/Pixel builds. If a vendor ROM removes
            // or changes it, Zygisk simply cannot provide a portable hook for that ROM.
            JNINativeMethod drmMethods[] = {
                {"getPropertyByteArray", "(Ljava/lang/String;)[B", (void*)hooked_getPropertyByteArray}
            };
            api->hookJniNativeMethods(env, "android/media/MediaDrm", drmMethods, 1);
            __android_log_print(ANDROID_LOG_INFO, LOG_TAG,
                                "MediaDrm native hook requested for deviceUniqueId");
        }
    }
    ...
    void onLoad(zygisk::Api* api, JNIEnv* env) override {
        ...
        s_media_drm_level = spoof_info.media_drm_level;
        ...
        // Do NOT DLCLOSE after installing a JNI hook: the hook points into this .so.
    }
...
std::string COPGVDModule::s_media_drm_level;
